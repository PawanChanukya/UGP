...
optimizer = custom_optim.SAM(model.parameters(), base_optimizer, lr=0.01)
param_history = []
...
for param in param_history:
    ...
    criterion = nn.BCEWithLogitsLoss()
    base_optimizer = torch.optim.SGD  # You can use any optimizer here
    optimizer = custom_optim.SAM(model.parameters(), base_optimizer, lr=0.001)
    for epoch in range(n_epochs):
        model.train()
...
with open(os.path.join(save_dir, 'param_history_sam.pkl'), 'wb') as f:
    pickle.dump(param_history, f)

with open(os.path.join(save_dir, 'poison_points_sam.pkl'), 'wb') as f:
