...
for param in param_history:
    ...
    criterion = nn.BCEWithLogitsLoss()
    base_optimizer = torch.optim.RMSprop  # You can use any optimizer here
    optimizer = SAM(model.parameters(), base_optimizer, lr=0.001, momentum = 0.9)
    for epoch in range(n_epochs):
        model.train()
...
with open(os.path.join(save_dir, 'param_history_sam_rmsprop.pkl'), 'wb') as f:
    pickle.dump(param_history, f)

with open(os.path.join(save_dir, 'poison_points_sam_rmsprop.pkl'), 'wb') as f:
