...
parser = argparse.ArgumentParser(description='Train spiral classifier and save parameter history and poison points')
parser.add_argument('--param-file', type=str, default='param_history.pkl', 
                    help='Filename for saving parameter history (default: param_history.pkl)')
parser.add_argument('--poison-file', type=str, default='poison_points.pkl', 
                    help='Filename for saving poison points (default: poison_points.pkl)')
parser.add_argument('--n-cycles', type=float, default=1.5,
                    help='Number of cycles in the spiral (default: 1.5)')
parser.add_argument('--n-points', type=int, default=200,
                    help='Number of points in the spiral (default: 200)')
parser.add_argument('--noise', type=float, default=0.2,
                    help='Standard deviation of noise (default: 0.2)')
parser.add_argument('--train-ratio', type=float, default=0.5,
                    help='Ratio of training data (default: 0.5)')
parser.add_argument('--holdout-ratio', type=float, default=0.2,
                    help='Ratio of holdout data (default: 0.2)')
...
# Create filenames with train and holdout ratios
param_filename = f"{args.param_file.split('.')[0]}_{args.train_ratio}_{args.holdout_ratio}.pkl"
poison_filename = f"{args.poison_file.split('.')[0]}_{args.train_ratio}_{args.holdout_ratio}.pkl"

print(f"Saving parameter history to {os.path.join(save_dir, param_filename)}")
print(f"Saving poison points to {os.path.join(save_dir, poison_filename)}")

# Save param_history and poison_points
with open(os.path.join(save_dir, param_filename), 'wb') as f:
    pickle.dump(param_history, f)

with open(os.path.join(save_dir, poison_filename), 'wb') as f:
