...
def compute_hvp(loss_fn, model, inputs, targets, v):
    ...
    """
    loss = loss_fn(model(inputs), targets)  # Compute loss using input data
...
def create_heat_map(model, loss_fn, train_inputs, train_labels, x_range, y_range, num_points=50):
    ...
    lambda_max, v_max_dict, lambda_min, v_min_dict = compute_hessian_directions(loss_fn, model, train_inputs, train_labels, num_iters=100)
        # saving the directions

    if not os.path.exists("directions/spiral"):
        os.makedirs("directions/spiral")

    with open("directions/spiral/v_max.pkl", "wb") as f:
        pickle.dump(v_max_dict, f)
    
    with open("directions/spiral/v_min.pkl", "wb") as f:
    ...
    for i, alpha in enumerate(alphas):
        for j, beta in enumerate(betas):
            perturbed_params = {}
            for name, param in model.named_parameters():
                perturbed_params[name] = original_params[name] + alpha * v_max_dict[name] + beta * v_min_dict[name]
            ...
            # Compute Hessian at new parameters
            ...
            ratio = abs(lambda_min_new / lambda_max_new)
            heat_map[i, j] = ratio
    ...
    plt.savefig(f"plots/heat_map_hessian_directions_{x_range[0]}_{x_range[1]}_{y_range[0]}_{y_range[1]}.png")
    plt.show()
